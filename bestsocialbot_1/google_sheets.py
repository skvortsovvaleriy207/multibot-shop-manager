...
SHEET_ORDERS = "Заявки"  # Новый лист для заявок
...
async def sync_order_requests_to_sheets():
    ...
    try:
        ...
        async with aiosqlite.connect("bot_database.db") as db:
            ...
            cursor = await db.execute("""
                SELECT 
                    'P' || r.id as prefixed_id,  -- Добавляем префикс 'P' для товаров/предложений
                    r.created_at, 
                    r.user_id, 
        ...
        for i, row in enumerate(existing_data[1:], start=2):  # start=2 потому что строка 1 - заголовки
            if row and len(row) > 0 and row[0]:  # Проверяем, что строка и ID не пустые
                try:
                    request_id = str(row[0]).strip()  # Теперь ID как строка с префиксом
        ...
        # Получаем ID из базы данных (теперь с префиксами)
        db_ids = set()
        for req in all_requests:
            try:
                db_ids.add(str(req[0]).strip())  # Преобразованный ID с префиксом
