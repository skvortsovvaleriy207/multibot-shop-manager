        ...
        async with aiosqlite.connect("bot_database.db") as db:
            ...
            for row in all_data:
                try:
                    ...
                    user_data = {
                        "user_id": user_id,
                        "username": row.get('Телеграм @username', ''),
                        "full_name": row.get('ФИО и возраст подписчика', ''),
                        "birth_date": '', # Removed from sheet
                        "location": row.get('Место жительства', ''),
                        "email": row.get('Email', ''),
                        "phone": '', # Removed from sheet
                        "employment": row.get('Текущая занятость', ''),
                        ...
                        "passive_subscriber": row.get('Пассивный подписчик (1.0)', ''),
                        "active_partner": row.get('Активный партнер (2.0)', ''),
                        "investor_trader": row.get('Инвестор/трейдер (3.0)', ''),
                        "business_proposal": row.get('Бизнес-предложение', ''),
                        "bonus_total": _safe_float(row.get('ИТОГО бонусов', 0)),
                        "current_balance": _safe_float(row.get('ТЕКУЩИЙ БАЛАНС', 0)),
                        "problem_cost": row.get('Стоимость проблем', ''),
                        "notes": row.get('Иная информация', ''), # Mapped to Notes (Иная информация)
                        "account_status": row.get('Статус аккаунта (Р/Б)', 'Р'),
                        "updated_at": datetime.now().isoformat()
                    }
