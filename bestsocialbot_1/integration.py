...
class GoogleSheetsIntegrator:
    ...
    async def integrate_to_autoavia_sheets_only(self, user_id: int) -> Dict[str, Any]:
        """
        Интегрирует данные пользователя только в Google Sheets бота Autoavia
        ...
        try:
            ...
            # Записываем данные в таблицу Autoavia
            sheets_success = await self._write_to_target_sheets(user_data, "autoavia")

            if sheets_success:
                return {
                    "success": True,
                    "message": "Данные успешно интегрированы в Google Sheets бота Autoavia",
                    "user_id": user_id,
                    "target_bot": "autoavia",
                    "sheets_sync": True
                }
                ...
                return {
                    "success": False,
                    "message": "Ошибка интеграции в Google Sheets бота Autoavia",
                    "target_bot": "autoavia",
                    "sheets_sync": False
                }

        except Exception as e:
            logging.error(f"Ошибка интеграции в Autoavia Sheets: {e}")
            return {
                "success": False,
                "message": f"Ошибка интеграции: {str(e)}",
                "target_bot": "autoavia"
