...
async def search_offers_by_category_execute(callback: CallbackQuery):
    ...
    # Выполняем поиск по категории в order_requests
    results = []
    ...
    builder.row(types.InlineKeyboardButton(text="◀️ В каталог", callback_data="offers_catalog"))
...
async def search_offers_by_class_execute(callback: CallbackQuery):
    ...
    # Выполняем поиск по классу в order_requests
    results = []
...
async def search_offers_by_type_execute(callback: CallbackQuery):
    ...
    # Выполняем поиск по типу в order_requests
    results = []
    ...
    async with aiosqlite.connect("bot_database.db") as db:
        ...
        if items:
            for i in items:
                ...
                builder.add(types.InlineKeyboardButton(
                    text=view_name,
                    callback_data=f"offer_view_search:{view_name}"
                ))
        else:
            builder.add(types.InlineKeyboardButton(
...
@dp.callback_query(F.data.startswith("offer_view_search:"))
async def search_offers_by_kind_execute(callback: CallbackQuery):
    ...
    # Выполняем поиск по виду в order_requests
    results = []
...
async def process_search_in_offers(message: Message, state: FSMContext):
    ...
    if not results:
        ...
        builder.add(types.InlineKeyboardButton(text="◀️ В каталог", callback_data="offers_catalog"))
        builder.adjust(1)
...
async def view_search_result_item(callback: CallbackQuery):
    ...
    builder.add(types.InlineKeyboardButton(text="💬 Связаться", callback_data=f"contact_seller_{item[1]}"))
