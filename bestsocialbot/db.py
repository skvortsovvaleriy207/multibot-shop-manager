...
SHARED_DB_FILE = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "shared_storage", "bot_database.db"))
print(f"DEBUG: db.py loaded. SHARED_DB_FILE = {SHARED_DB_FILE}")
...
async def connect_db():
    async with aiosqlite.connect(DB_FILE) as db:
        ...
        # ATTACH SHARED DB
        try:
            await db.execute(f"ATTACH DATABASE '{SHARED_DB_FILE}' AS shared")
        except Exception as e:
            print(f"Error attaching shared DB in connect_db: {e}")
...
async def init_db():
    ...
    try:
        async with aiosqlite.connect(SHARED_DB_FILE) as shared_db:
            ...
            print("Shared DB initialized successfully (bestsocialbot)")
    except Exception as e:
    ...
    async with connect_db() as db:
            ...
            # ATTACH SHARED DB (Already handled in connect_db, but safe to verify or double-check specific init logic)
            # Keeping specific init logic...
            ...
            # Проверка и добавление колонки bot_subscriptions если её нет (миграция)
            try:
                await db.execute("ALTER TABLE users ADD COLUMN bot_subscriptions TEXT")
                print("Added column bot_subscriptions to users table")
            except Exception:
                pass # Колонка уже существует
            ...
            if count == 0 and False: # Disabled initial insert
